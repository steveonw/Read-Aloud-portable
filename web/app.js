...
async function play(samples, sampleRate) {
  ...
  source.onended = () => {
    if (currentSource === source) {
      currentSource = null;
      setStatus('ready', 'Amy is ready', 'Highlight another sentence and press F8.');
      updateButtons();
    }
  };
...
function stopPlayback() {
  ...
  setStatus('ready', 'Amy is ready', 'Highlight a sentence and press F8.');
  updateButtons();
}
...
function requestSpeech() {
  ...
  generating = true;
  updateButtons();
  setStatus('loading', 'Generating speech…', text.length > 180 ? 'Long selections take a little longer.' : 'Amy is preparing the sentence.');
  worker.postMessage({
    type: 'generate',
    text,
    sid: 0,
  ...
  switch (message.type) {
    case 'sherpa-onnx-tts-progress': {
      ...
      if (match) {
        ...
        setStatus('loading', `Loading Amy… ${percent}%`, 'Reading the offline model from this drive.');
      } else {
        setStatus('loading', 'Loading Amy…', raw.replace('Running...', 'Initializing the voice model…'));
      }
      break;
    }
