...
OUTPUT_ROOT="$PROJECT_ROOT/dist/ReadAloudUSB"
...
rm -f "$PROJECT_ROOT/dist/ReadAloudUSB.zip"
(
  cd "$PROJECT_ROOT/dist"
  zip -q -r -9 ReadAloudUSB.zip ReadAloudUSB
)

printf 'USB folder: %s\n' "$OUTPUT_ROOT"
printf 'ZIP archive: %s\n' "$PROJECT_ROOT/dist/ReadAloudUSB.zip"
